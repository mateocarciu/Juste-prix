...
export async function createGame(userId, reply) {
	if (!userId) {
		return reply.status(400).send({ error: "L'identifiant du joueur est manquant" }) // 400 Bad Request
	}
	...
	switch (action) {
		...
		case 'start':
			if (!game.player1 || !game.player2) {
				return reply.status(400).send({ error: 'Deux joueurs sont requis pour démarrer la partie.' })
			}
			game.state = 'playing'
			break
		...
		case 'finish':
			if (!request.body.score || !request.body.winner) {
				return reply.status(400).send({ error: 'Le score et le gagnant sont requis pour terminer la partie.' })
			}
			game.state = 'finished'
...
export async function getOpenGames(reply) {
	try {
		...
		console.error('Erreur lors de la récupération des parties ouvertes :', error)
		return reply.status(500).send({ error: 'Erreur serveur lors de la récupération des parties.' })
	}
}
...
export async function getFinishedGames(userId, reply) {
	try {
		...
		console.error('Erreur lors de la récupération des parties terminées :', error)
		return reply.status(500).send({ error: 'Erreur serveur lors de la récupération des parties.' })
	}
}
