import { createGame, updateGame, getGameState, getOpenGames, getFinishedGames } from '../controllers/games.js'
...
export function gamesRoutes(app) {
	...
	app.get('/games/open', { preHandler: [app.authenticate] }, async (request, reply) => {
		return await getOpenGames(reply)
	})
	app.post('/games/finished', { preHandler: [app.authenticate] }, async (request, reply) => {
